...
const OPENAI_API_KEY = ""; // можно вставить ключ только для тестов
...
function awardStar(sectionKey, mode = "offline") {
  ...
  } else if (currentUnitTest) {
    journal[idx].lastTopic = "Unit Test " + currentUnitTest;
  } else {
...
const UNIT_TESTS = {
  "1-2": {
    title: "Test 1–2: Let’s Have Fun! + Sports and Exercises",
    questions: [
      {
        id: "q1",
        type: "mc",
        question: "They’re ______ hide-and-seek in the park.",
      ...
      },
      {
        id: "q3",
        type: "mc",
        question: "Does Oraz play ice hockey?",
        options: ["Yes, he does.", "No, he doesn’t.", "Yes, they do."],
        correct: "Yes, he does."
      }
    ]
  },
  "3-4": {
    title: "Test 3–4: Sports Around the World + Arts and Music",
    questions: [
      {
        id: "q1",
        type: "mc",
        question: "In Canada many people play ______.",
        options: ["table tennis", "ice hockey", "volleyball"],
        correct: "ice hockey"
      },
      {
        id: "q2",
      ...
      },
      {
        id: "q3",
        type: "mc",
        question: "A dombyra is a ______.",
        options: ["sport", "instrument", "place"],
        correct: "instrument"
      }
    ]
  },
  "5-6": {
    title: "Test 5–6: Team Sports + Museum",
    questions: [
      {
        id: "q1",
        type: "mc",
        question: "The person who stops the ball is the ______.",
        options: ["referee", "goalkeeper", "player"],
        correct: "goalkeeper"
      },
      {
        id: "q2",
      ...
      },
      {
        id: "q3",
        type: "mc",
        question: "There ______ a big dombyra in the museum.",
        options: ["is", "are", "be"],
        correct: "is"
      }
    ]
  ...
  "7-8": {
    title: "Test 7–8: Children’s Rights + Different Jobs",
    questions: [
      {
        id: "q1",
        type: "mc",
        question: "Children have the right to ______.",
        options: ["go to school", "drive a bus", "work at night"],
        correct: "go to school"
      },
      {
        id: "q2",
        type: "mc",
        question: "Children ______ help at home.",
        options: ["must", "mustn’t", "can’t"],
        correct: "must"
      },
      {
        id: "q3",
        type: "mc",
        question: "A vet works with ______.",
        options: ["cars", "animals", "computers"],
        correct: "animals"
      }
    ]
  },
  "9-10": {
    title: "Test 9–10: Professions + People at Work",
    questions: [
      {
        id: "q1",
        type: "mc",
        question: "Where does a doctor usually work?",
        options: ["at a hospital", "at a farm", "at an airport"],
        correct: "at a hospital"
      },
      {
        id: "q2",
        type: "mc",
      ...
      },
      {
        id: "q3",
        type: "mc",
        question: "A firefighter can ______.",
        options: ["fly planes", "put out fires", "teach English"],
        correct: "put out fires"
      }
    ]
  },
  "11-12": {
    title: "Test 11–12: What Do You Do? + You Can Help!",
    questions: [
      {
        id: "q1",
        type: "mc",
        question: "We can ______ the park.",
        options: ["clean", "dirty", "break"],
        correct: "clean"
      },
      {
        id: "q2",
        ...
        options: [
          "We will clean the park next weekend.",
          "We cleaned the park next weekend.",
          "We clean the park next weekend."
        ],
        correct: "We will clean the park next weekend."
      },
      {
        id: "q3",
        type: "mc",
        question: "Recycling helps ______.",
        options: ["the planet", "eat more", "sleep late"],
        correct: "the planet"
      }
    ]
  }
...
let currentUnitAnswers = {};

// ===== TOPIC OPEN =====
function openTopic(num) {
  currentTopic = num;
  currentUnitTest = null; // выходим из режима тестов
  ...
  const unitSec = document.getElementById("unitTestsSection");
...
function openUnitTests() {
  currentTopic = null; // выходим из темы
  const chat = document.getElementById("chatSection");
  ...
  if (unitSec) unitSec.style.display = "block";

  renderUnitTestsMenu();
}

function renderUnitTestsMenu() {
  const menu = document.getElementById("unitTestsMenu");
  const content = document.getElementById("unitTestsContent");
  if (!menu || !content) return;

  menu.innerHTML = `
    <p>Select a test:</p>
    <button class="btn" onclick="startUnitTest('1-2')">Test 1–2</button>
    <button class="btn" onclick="startUnitTest('3-4')">Test 3–4</button>
    <button class="btn" onclick="startUnitTest('5-6')">Test 5–6</button>
    <button class="btn" onclick="startUnitTest('7-8')">Test 7–8</button>
    <button class="btn" onclick="startUnitTest('9-10')">Test 9–10</button>
    <button class="btn" onclick="startUnitTest('11-12')">Test 11–12</button>
  `;
  content.innerHTML = `<p>After you choose a test, answer all questions and click "Submit Test".</p>`;
}

function startUnitTest(key) {
  const test = UNIT_TESTS[key];
  if (!test) return;
  currentUnitTest = key;
  currentUnitAnswers = {};
  const content = document.getElementById("unitTestsContent");
  ...
  html += `
    <button class="btn" onclick="submitUnitTest()">Submit Test ✅</button>
    <p id="unitTestResult" style="margin-top:8px;"></p>
  `;
  content.innerHTML = html;
}

function submitUnitTest() {
  if (!currentUnitTest) return;
  const test = UNIT_TESTS[currentUnitTest];
  if (!test) return;
  ...
  const resultEl = document.getElementById("unitTestResult");
  if (!resultEl) return;

  resultEl.textContent =
    `You got ${correctCount} / ${test.questions.length} correct.`;
  if (correctCount === test.questions.length) {
    resultEl.textContent += " ✅ Excellent! You get 1 star.";
    resultEl.style.color = "green";
    awardStar("quiz", "offline");
  } else {
    resultEl.textContent += " Try again or ask your teacher.";
